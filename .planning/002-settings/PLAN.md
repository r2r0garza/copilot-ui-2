...
**Status:** Planned
**Created:** 2026-07-29
**Scope:** Workspace-level workbench settings for default agent selection and DeepAgents prompt composition
...
### Plan 05 — Verification and diagnostics

- Extend the model-call diagnostic output to identify whether the DeepAgents base prompt was included.
...
- Default settings are `defaultAgentId = null` and `includeDeepAgentsDefaultSystemPrompt = false`.
...
6. Turn the DeepAgents base prompt off, send a message, and inspect the model-call log.
7. Turn it on, send another message in the same chat, and verify the log includes the base prompt after the project-agent instructions.
8. Verify tool availability and approval behavior are unchanged in both modes.
9. Restart the Extension Host and verify both settings persist.
